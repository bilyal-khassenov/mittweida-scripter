...
def main():
    ...
    with st.form(key="Form :", clear_on_submit = False):
        ...
        summary_setting = st.checkbox("Zusammenfassung generieren", value=False)
    ...
    if submit_button:
        if email_address_textbox == '':
            ...
            with st.spinner(texts_from_config_file['uploading_file_process_spinner_label']):  # Show spinner
                ...
                #Summary Setting
                summary_setting = "1" if summary_setting else "0"
                ...
                plain_structured_original_file_name_stem = f"{datetime.today().strftime('%Y%m%d#%H%M%S')}#{email_address_textbox}#{language_setting}#{translation_setting}#{diarization_setting}#{subtitle_setting}#{summary_setting}#{transcription_model_setting}#{userdefined_file_name_stem}"[0:120]
