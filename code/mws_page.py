    ...
    if submit_button:
        if email_address_textbox == '':
            ...
            with st.spinner(texts_from_config_file['uploading_file_process_spinner_label']):  # Show spinner
                ...
                new_file_name_stem = f"{datetime.today().strftime('%Y%m%d#%H%M%S')}#{email_address_textbox}#{language_setting}#{translation_setting}#{diarization_setting}#{summary_setting}#{transcription_model_setting}#{file_name_stem}"[
                    0:120]
